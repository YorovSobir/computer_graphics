...
void main (void)
{
	...
	s[0] = texture(tex, Tap[0]);
	s[1] = texture(tex, Tap[1]);
	s[2] = texture(tex, Tap[2]);
	s[3] = texture(tex, Tap[3]);
	s[4] = texture(tex, TapNeg[0]);
	s[5] = texture(tex, TapNeg[1]);
	s[6] = texture(tex, TapNeg[2]);
	...
	// Sample the taps
	s[0] = texture(tex, Tap4[0]);
	s[1] = texture(tex, Tap4[1]);
	s[2] = texture(tex, Tap4[2]);
	s[3] = texture(tex, TapNeg4[0]);
	s[4] = texture(tex, TapNeg4[1]);
	s[5] = texture(tex, TapNeg4[2]);
	...
	FragColor = vec4(ColorSum, WeightSum);
}